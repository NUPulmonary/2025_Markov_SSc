...
OUT_DIR <- '/projects/b1038/Pulmonary/nmarkov/ssc-v2/data/31_bal-object/11_pseudobulk-v3/'
...
process_cell_type <- function(cell_type) {
  ...
  # Select only V3 chemistry
  coldata <- coldata[coldata$Chemistry == "10x 3' V3", ]
  ...
  if (count(coldata$Status == 'SSc') < 2) {
    print("Only one or two SSc samples, skipping")
    return()
  }

  if (count(coldata$Status == 'control') < 2) {
  ...
  dds <- DESeqDataSetFromMatrix(counts, coldata, ~ Status + Sex)
  ...
  status_sex_dir <- sprintf("%s/%s/degs-status-sex", OUT_DIR, cell_type)
  ...
  plot(ggplot(d, aes(
    ...
    scale_color_manual(values = c("control" = "blue", "SSc" = "red")) +
  ...
  degs <- as.data.frame(results(dds, contrast = c("Status", "control", "SSc"), alpha = 0.05))
  ...
  degs$sign[degs$sign == "Downregulated"] <- sprintf("Up in SSc (%d)", n_downreg)
  n_upreg <- sum(degs$sign == "Upregulated")
  degs$sign[degs$sign == "Upregulated"] <- sprintf("Up in Control (%d)", n_upreg)
